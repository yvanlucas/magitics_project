...
class KmerExtractionAndCount(object):
    ...
    Marius Erbert, Steffen Rechner, and Matthias Müller-Hannemann, Gerbil:
     A fast and memory-efficient k-mer counter
    ...
    def __init__(self, fastaname):
        ...
        self.pathtotemp = os.path.join(
            cfg.pathtoxp, cfg.xp_name, "temp", self.label + self.strainnumber
        )
        # self.pathtosavetemp = os.path.join(cfg.pathtoxp, cfg.xp_name , 'kmers/temp', self.strainnumber)
        self.pathtosave = os.path.join(
            cfg.pathtoxp, cfg.xp_name, "kmers", self.label + self.strainnumber
        )
    ...
    def parse_kmers_gerbil(self):
        ...
        os.system(
            kmerCmd
        )  # default minimal abundance of kmers=3, can be changed with "-l newvalue"
        tofastaCmd = "toFasta %s %d %s" % (
            self.pathtosavetemp,
            self.len_kmers,
            self.pathtosave,
        )
...
class KmersCounts2Dataframe(object):
            ...
            for filename in os.listdir(os.path.join(cfg.pathtodata, cfg.data, dirname)):
                ...
                for key in kmer.kmer_counts.keys():
                    if key in self.kmerdicts:
                        self.kmerdicts[key][kmer.strainnumber] = int(
                            kmer.kmer_counts[key]
                        )
                    else:
                        self.kmerdicts[key] = {
                            kmer.strainnumber: int(kmer.kmer_counts[key])
                        }
    ...
    def create_sparse_matrix(self):
        ...
        if not self.kmerdicts:
            ...
            with open(
                os.path.join(cfg.pathtoxp, cfg.xp_name, "kmerdicts.pkl"), "rb"
            ) as f:
                self.kmerdicts = pickle.load(f)

        n_strains = len(self.strains)
        self.strain_to_index = {
            strain: i for i, strain in zip(range(n_strains), self.strains)
        }
        ...
        self.mat = sp.csr_matrix(
            (data, (rows, columns)),
            shape=(n_strains, len(self.kmer_to_index)),
            dtype=np.int8,
        )

        with open(os.path.join(cfg.pathtoxp, cfg.xp_name, "kmers_mats.pkl"), "wb") as f:
            pickle.dump(
                [self.mat, self.labels, self.strain_to_index, self.kmer_to_index],
                f,
                protocol=4,
            )
    ...
    def create_dataframe(self):
        ...
        pq.write_table(
            table, os.path.join(cfg.pathtoxp, cfg.xp_name, "kmers_DF.parquet")
        )
