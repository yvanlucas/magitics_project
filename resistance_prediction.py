...
from sklearn import (ensemble, feature_selection, metrics, model_selection,
                     preprocessing)
...
class ResistancePredictionkmers(object):
    def __init__(self, dataframe=None, classifier=None, param_grid=None):
        ...
        self.chi2_selector = feature_selection.SelectKBest(
            feature_selection.chi2, k=1000000
        )
    ...
    def eval(self, y_test, y_pred, X_test):
        ...
        self.pvt = pd.pivot_table(
            pd.DataFrame(self.cv_clf.cv_results_),
            values="mean_test_score",
            index="param_" + ls_params[0],
            columns="param_" + ls_params[1],
        )  # Only works if 2 params in grid

        ax = sns.heatmap(self.pvt)
        ax.set(ylabel=ls_params[0], xlabel=ls_params[1])
        ax.figure.savefig(
            os.path.join(
                cfg.pathtoxp, cfg.xp_name, cfg.id, f"{cfg.model}_gridCV_heatmap.png"
            )
        )
        ...
        if cfg.model == "gradient":
            test_score = np.zeros(
                (self.cv_clf.best_params_["n_estimators"],), dtype=np.float64
            )
            for i, y_pred in enumerate(
                self.cv_clf.best_estimator_.staged_predict(X_test)
            ):
            ...
            plt.plot(
                np.arange(self.cv_clf.best_params_["n_estimators"]) + 1,
                self.cv_clf.best_estimator_.train_score_,
                "b-",
            ...
            plt.savefig(
                os.path.join(
                    ...
                    cfg.xp_name,
                    cfg.id,
                    f"{cfg.model}boosting_learning_curve.png",
                )
            )

    def write_report(self):
        with open(
            os.path.join(cfg.pathtoxp, cfg.xp_name, cfg.id, f"{cfg.model}_report.txt"),
            "w",
        ) as txt:
            ...
            if cfg.model == "rf" or cfg.model == "gradient":
                ...
                kmers = [
                    list(self.kmer_to_index.keys())[i] for i in np.nonzero(featimp)[0]
                ]
                for kmer in kmers:
                    txt.write(str(kmer) + "\n")
    ...
    def dump_eval(self, y_test, y_predict):
        with open(
            os.path.join(
                cfg.pathtoxp, cfg.xp_name, cfg.id, f"{cfg.model}_CVresults.pkl"
            ),
            "wb",
        ) as f:
            pickle.dump(
    ...
    def run(self, evaluate=True):
        ...
        with open(
            ...
            pickle.dump(
                {"classifier": self.cv_clf, "features": self.kmer_to_index},
                f,
                protocol=4,
            )
        print("3")
...
class TestStreamingBatch(object):
    ...
    def predict_fastas(self):
        ...
        while remaining > 0:
            ...
            X_test = sp.csr_matrix(
                (datas, (rows, cols)),
                shape=(batchsize, len(self.kmer_to_index)),
                dtype=np.int8,
            )
    ...
    def parse_and_map_kmers(self, fastaname, batchnumber):
        ...
        kmerCmd = "dsk -file %s -out %s -kmer-size %d -abundance-min 1 -verbose 0" % (
            os.path.join(self.testdir, fastaname),
            os.path.join(self.pathtotemp, fastaname),
            cfg.len_kmers,
        )
        os.system(kmerCmd)
        outputCmd = "dsk2ascii -file %s -out  %s" % (
            os.path.join(self.pathtotemp, fastaname),
            os.path.join(self.pathtosave, fastaname),
        )
        os.system(outputCmd)
        label = fastaname[:5]
        self.kmer_count = {}
    ...
    def evaluate(self):
        ...
        self.score["Accuracy"] = metrics.accuracy_score(
            y_test, self.y_preds[:, 1].round()
        )
...
with open(
    os.path.join(cfg.pathtoxp, cfg.xp_name, cfg.id, f"{cfg.model}_CVresults.pkl"), "rb"
) as f:
    dic = pickle.load(f)
    clf = dic["classifier"]
